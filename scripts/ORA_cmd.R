...
DATABASES=c("geneontology_Biological_Process", "geneontology_Molecular_Function") 
...
for (DATABASE in DATABASES) {
    ...
    if (dir.exists(INPUT_PATH)) {
        significantModules <- list.files(path = INPUT_PATH, pattern = "GS_*.txt", full.names = TRUE)
        if (length(significantModules) == 0) {
            print("NO SIGNIFICANT MODULES")
            write.csv(NULL,file.path(SUMMARIES_PATH,"dummy.csv"),row.names = FALSE)
        }
        print(significantModules)
        for (significantModule in significantModules) {
            name = unlist(strsplit(significantModule, '.txt'))[1]
